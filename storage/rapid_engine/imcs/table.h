...
class RapidTable : public MemoryObject {
 public:
   ...
   */
  struct SHANNON_ALIGNAS TableStats {
...
class RpdTable : public MemoryObject {
 public:
   ...
   */
  virtual row_id_t insert_row(const Rapid_load_context *context, const uchar *data) = 0;
   ...
   * @return Inserted global row_id, returns INVALID_ROW_ID on failure.
   */
  virtual row_id_t locate_row(const Rapid_load_context *context, const uchar *rowdata) = 0;
   ...
   * Compress IMCUs
   */
  virtual size_t compact_imcus(double delete_ratio_threshold = 0.5) = 0;
 ...
 protected:
  ...
  virtual Imcu *locate_imcu(row_id_t global_row_id) = 0;
  ...
  struct SHANNON_ALIGNAS Imcu_Index {
    ...
    Imcu_Index()
        : min_values(MAX_COLUMNS, SHANNON_MAX_DOUBLE),
          max_values(MAX_COLUMNS, SHANNON_MIN_DOUBLE),
          has_deletes(false),
          delete_ratio(0.0) {}
...
class Normal_Table : public RpdTable {
 public:
  Normal_Table(const TABLE *&mysql_table, const TableConfig &config);
  virtual ~Normal_Table() {}
  ...
  virtual row_id_t insert_row(const Rapid_load_context *context, const uchar *data) override;
  ...
  virtual row_id_t locate_row(const Rapid_load_context *context, const uchar *rowdata) override;
  ...
  virtual size_t compact_imcus(double delete_ratio_threshold = 0.5) override;

  virtual bool reorganize() override;

  virtual Imcu *locate_imcu(row_id_t global_row_id) override {
    size_t imcu_idx = global_row_id / m_metadata.rows_per_imcu;
    if (imcu_idx >= m_imcus.size()) return nullptr;
    return m_imcus[imcu_idx].get();
  }
...
class RpdPartTable : public RpdTable {
 public:
  RpdPartTable(const TABLE *&mysql_table, const TableConfig &config) : RpdTable(mysql_table, config) {}
  virtual ~RpdPartTable() {}
  ...
  virtual row_id_t insert_row(const Rapid_load_context *context, const uchar *data) override { return INVALID_ROW_ID; }
  ...
  virtual size_t compact_imcus(double delete_ratio_threshold = 0.5) override { return 0; }

  virtual bool reorganize() override;

  virtual Imcu *locate_imcu(row_id_t global_row_id) override { return nullptr; }
